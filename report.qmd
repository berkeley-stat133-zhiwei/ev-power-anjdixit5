...
ev_use <- read.csv("data/ev-registrations-by-state-2023.csv") |>
  clean_names() |>
  slice(-(1:2)) |> #getting rid of blank/unnecessary rows
  rename(
    state = electric_vehicle_registrations_by_state_2023,
    ev_count_2023 = x #renaming columns
  ) |>
    mutate(ev_count_2023 = str_extract(ev_count_2023, "\\d+\\.?\\d*") |> 
    as.numeric()) #making all ev counts purely numeric
head(ev_use) #double checking
...
renew_use <- read.csv("data/renew-use-2023.csv") |>
  ...
  mutate(
    state = coalesce(state_full, str_to_title(state)),  # convert abbreviations
    renewable_use_2023 = readr::parse_number(renewable_use_2023)
  ) |>
  select(-state_full)
renew_use_summary <- renew_use |>
  group_by(state) |>
  summarise(
    total_renewable_2023 = sum(renewable_use_2023, na.rm = TRUE),
    .groups = "drop"
  )
head(renew_use)
...
States with higher renewable generation (e.g., California, Texas) also tend to have more EVs, suggesting a positive relationship. However, this correlation is not perfect — some states with high renewable capacity have fewer EVs, possibly due to differences in policy or infrastructure.